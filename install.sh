...
adduser -D -u 1000 junv \
  && apk update \
  && apk add runit shadow wget bash curl openrc gnupg aria2 tar mailcap vim jq findutils --no-cache \
  ...
  && ln -s /data/cloud /app \
  && mkdir -p /data/upload \
  && chown junv:junv /data/upload \
  && ln -s /data/upload /app \
  && chown junv:junv /app/conf/* \
  && chmod 755 /app/conf/* 
